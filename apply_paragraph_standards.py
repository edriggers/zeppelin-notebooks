...
-l, --lint switch will run the script in "lint" mode, returning a non-zero value
      on failure to pass lint.
...
def process_paragraph(paragraph, tracking, test_exceptions):
    ...
    routines to be called to set things like 'editorSettings/language' and
...
def main():
    ...
    parser.add_argument("-l", "--lint",
                        help="switch to run script in 'lint' mode",
                        action="store_true")
    ...
    # Lint and Modify are conflicting, if we indicate Lint, then no modification.
    if args.lint:
        print("WARNING: Cannot -m (modify) when -l (lint) is specified")
        args.modify = False
    track_vars = {
        'modify': args.modify,
